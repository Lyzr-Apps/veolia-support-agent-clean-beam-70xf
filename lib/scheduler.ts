...
export async function pauseSchedule(scheduleId: string): Promise<ApiResult> {
  ...
  try {
    ...
    // If API returns 400 with "already paused/inactive", treat as success
    if (!data.success && data.details) {
      const details = typeof data.details === 'string' ? data.details.toLowerCase() : ''
      if (details.includes('already') && (details.includes('paused') || details.includes('inactive'))) {
...
export async function resumeSchedule(scheduleId: string): Promise<ApiResult> {
  ...
  try {
    ...
    // If API returns 400 with "already active", treat as success
    if (!data.success && data.details) {
      const details = typeof data.details === 'string' ? data.details.toLowerCase() : ''
      if (details.includes('already active')) {
        return { success: true, data }
      }
    }
